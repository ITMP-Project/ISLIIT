    ...
    >
      ...
      >
        ...
        <button
          ...
          @click="loadTimeTables"
        >
          Refresh
        </button>
      </div>
      <div v-if="loading" class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
        Loading student time table...
      </div>
      <div v-else-if="error" class="px-4 py-3 text-sm text-error-600 dark:text-error-400">
        {{ error }}
      </div>
...
const currentPageTitle = ref('Student Time Table')

const { timeTables, loading, error, fetchTimeTables, apiUrl } = useTimeTableStore()
...
const formatValue = (value) => (typeof value === 'string' ? value.trim() : '')

const resolveCalendarLevel = (type) => {
  const key = formatValue(type).toLowerCase()
  return timeTableTypeColors[key] ?? 'Primary'
}

const buildTimeTableEvent = (item) => {
  const date = formatValue(item?.date)
  if (!date) return null

  const startTime = formatValue(item?.startTime)
  const endTime = formatValue(item?.endTime)
  const allDay = !startTime || !endTime || (startTime === '00:00' && endTime === '23:59')
  const title = formatValue(item?.title) || 'Untitled Event'
  const courseCode = formatValue(item?.courseCode)
  const displayTitle = courseCode ? `${courseCode}: ${title}` : title
  const calendar = resolveCalendarLevel(item?.type)

  return {
    id: item?._id ?? `${date}-${displayTitle}`,
    title: displayTitle,
    start: allDay ? date : `${date}T${startTime}`,
    end: !allDay && endTime ? `${date}T${endTime}` : undefined,
    allDay,
    extendedProps: {
      calendar,
      type: item?.type,
...
const buildTimeTableEvents = (items) =>
  items.map(buildTimeTableEvent).filter(Boolean)

const loadTimeTables = async () => {
  await fetchTimeTables()
  events.value = buildTimeTableEvents(timeTables.value)
}
